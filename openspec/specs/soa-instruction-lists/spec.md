# soa-instruction-lists Specification

## Purpose
The IR is represented as `InstList` value-type structs containing parallel primitive arrays for homogeneous instruction fields, plus a variable-length operand pool. All cross-references use typed readonly-struct handles wrapping `int`.
...
- **THEN** first `MAX_FIXED_OPS` operands go to `fixedOps`, remaining operands are appended to the `extra` pool; `extraOffset[idx]` and `extraLength[idx]` record the slice.
