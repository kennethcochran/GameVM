...
- **Conditional branch polarity:** `if x <> 0 then y := 1` lowers through the pipeline as `Cmp(x,0); Transition; Branch(L_end)` (BEQ → skip when x==0) and `if x = 0 then y := 1` as `Cmp(x,0); Branch(L_end)` (BNE → skip when x!=0). The `Transition` marker inverts the `Branch-to-target-when-true` convention: `<>` (then-arm) uses no Transition → BNE; `=` (then-arm) uses Transition → BEQ. `ProcessBranch` emits `Transition` BEFORE `Branch` so the codegen's `lastWasTransition` flag selects BEQ/BNE correctly. `- **Atari 2600 codegen mechanics:** `GenerateFromSlab` emits 6502 opcodes — `LLIR_LOAD` → `LDA #imm` (`0xA9`) for a single-operand immediate; `LDA` zero-page (`0xA5`) when two operands form an address `< $100`, else absolute (`0xAD`). `LLIR_STORE` → `STA` zero-page (`0x85`) when addr `< $100`, else absolute (`0x8D`). `LLIR_SUB` → `SEC; SBC #imm` (`0x38 0xE9`). `LLIR_CMP` → `CMP #imm` (`0xC9`). A loaded cartridge has total machine control, so the emitted program never returns: codegen appends a **self-loop** (`JMP *` → `0x4C <self>`) so the CPU holds the final state instead of falling into zeroed ROM. Reset/IRQ vectors (`$FFFC`-`$FFFF`) point to `$F000`. Variables resolve via the shared `StringPool`: known TIA registers map to hardware (`COLUBK`→`$09`, `COLUPF`→`$08`, `COLUP0`→`$06`, `COLUP1`…

### Aspirational / Not Yet Implemented

- **Testing:** 490 tests, `dotnet test` must pass. SonarQube quality gate is enforced on CI.
